...
class _KycScreenState extends ConsumerState<KycScreen> {
  ...
  Future<void> _pickImage(bool isDni) async {
    ...
    // Compresión: quality 50, resolución máx 1200×1200 → imagen < 1 MB.
    // El DNI/selfie solo requiere legibilidad para revisión administrativa.
    ...
    if (img != null && mounted) {
      // Validación de tamaño post-compresión (umbral reducido a 1 MB).
      final bytes = await img.readAsBytes();
      if (bytes.length > 1 * 1024 * 1024) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('La imagen es demasiado grande (máx. 1MB)'),
